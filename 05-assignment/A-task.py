...
def point_2(fit, summary, fit_model):
    ...
    num_new_samples = 20
    sample_log_reaction_times = np.random.normal(new_person_theta, new_person_sigma, (num_new_samples, ))
    sample_reaction_times = np.exp(sample_log_reaction_times)

    plt.subplot(2, 1, 1)
    plt.scatter(np.arange(num_new_samples)+1, sample_reaction_times)
    plt.subplot(2, 1, 2)
    sns.distplot(sample_reaction_times, bins=10, norm_hist=True)
