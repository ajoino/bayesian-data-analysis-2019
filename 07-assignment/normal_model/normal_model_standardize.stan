...
transformed data {
	...
	vector[num_data] std_log_reaction_times;
	vector[num_data] std_attempt;
	log_reaction_times = log(reaction_times);
	std_log_reaction_times = (log_reaction_times - mean(log_reaction_times)) / sd(log_reaction_times);
	std_attempt = (attempt - mean(attempt)) / sd(attempt);
}
parameters {
    real adult_mean[2];
...
model {
	...
	for (datum in 1:num_data)
		log_reaction_summand[datum] = user_intersect[users[datum]] 
									+ user_slope[users[datum]] * std_attempt[datum];
		//log_reaction_times[datum] ~ normal(theta[users[datum]], sigma);
	std_log_reaction_times ~ normal(log_reaction_summand, sigma);
}
